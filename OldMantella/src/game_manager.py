...
class GameStateManager:
    ...
    def __init__(self, game: Gameable, chat_manager: ChatManager, config: ConfigLoader, language_info: dict[Hashable, str], client: LLMClient, stt_api_file: str, api_file: str):        
        ...
        # Queue for events that arrive after a radiant conversation ends
    ...
    def start_conversation(self, input_json: dict[str, Any]) -> dict[str, Any]:
        ...
        # Handle pending radiant events (OPTIMIZED: reduced overhead and logging)
        if self.__pending_radiant_events:
            # Check if this is a radiant conversation (no player)
            if not self.__talk.context.npcs_in_conversation.contains_player_character():
                # Add pending events to radiant event log
                logging.info(f"[RADIANT] Injecting {len(self.__pending_radiant_events)} queued events into new conversation")
                self.__talk.context.add_to_radiant_event_log(self.__pending_radiant_events)
            else:
                # This is a player conversation - clear pending radiant events as they're not relevant
                logging.debug(f"[PLAYER] Clearing {len(self.__pending_radiant_events)} pending radiant events")
            # Clear the queue in both cases
    ...
    def continue_conversation(self, input_json: dict[str, Any]) -> dict[str, Any]:
        if(not self.__talk ):
            # Check if there are events in this call - save them for next radiant conversation (OPTIMIZED: reduced logging)
            if input_json.__contains__(comm_consts.KEY_CONTEXT):
                if input_json[comm_consts.KEY_CONTEXT].__contains__(comm_consts.KEY_CONTEXT_INGAMEEVENTS):
                    ingame_events: list[str] = input_json[comm_consts.KEY_CONTEXT][comm_consts.KEY_CONTEXT_INGAMEEVENTS]
                    if isinstance(ingame_events, list) and len(ingame_events) > 0:
                        self.__pending_radiant_events.extend(ingame_events)
                        logging.info(f"[RADIANT] Queued {len(ingame_events)} late events for next conversation (total: {len(self.__pending_radiant_events)})")
                        # Return success - events are saved
    ...
    def end_conversation(self, input_json: dict[str, Any]) -> dict[str, Any]:
        if(self.__talk):
            # Note: Events are now properly cleared when used (either during conversation or in summary)
            # We no longer need to save "remaining" events because:
            # 1. Events sent to LLM during conversation are cleared immediately after use
            # 2. Events included in summary are cleared after summary is saved
            # 3. Any truly unused events will be re-sent by the game in the next conversation cycle
        ...
        if self.__talk:
            ...
            if json.__contains__(comm_consts.KEY_CONTEXT):
                ...
                if json[comm_consts.KEY_CONTEXT].__contains__(comm_consts.KEY_CONTEXT_INGAMEEVENTS):
                    ...
                    if isinstance(ingame_events, list) and len(ingame_events) > 0:
                        logging.log(28, f"Game Manager received {len(ingame_events)} events")
                    else:
                        logging.log(28, f"Game Manager received no events")
