...
class ChatManager:
    ...
    async def process_response(self, active_character: Character, blocking_queue: SentenceQueue, messages : message_thread, characters: Characters, actions: list[Action]):
        ...
        is_radiant = not characters.contains_player_character()
        
        # For radiant conversations, use single sentence config to get more frequent updates
        # For player conversations, use multi-NPC config if multiple NPCs present
        if is_radiant:
            max_response_sentences = self.__config.max_response_sentences_single
        else:
        ...
        parser_chain.extend([
            ...
            max_count_sentences_parser(max_response_sentences, is_radiant)
        ])
