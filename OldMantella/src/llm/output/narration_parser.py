...
class narration_parser(output_parser):
    ...
    def cut_sentence(self, output: str, current_settings: sentence_generation_settings) -> tuple[SentenceContent | None, str]:
        ...
        # Special handling for *Serana and *Lydia patterns - treat as speech, not narration
        if self.__is_serana_or_lydia_pattern(output):
            return self.__handle_serana_lydia_pattern(output, current_settings)
    ...
    def __is_serana_or_lydia_pattern(self, text: str) -> bool:
        """Check if text starts with *Serana or *Lydia pattern"""
        return (text.startswith('*Serana') or text.startswith('* Lydia') or
                text.startswith('**Serana') or text.startswith('** Lydia'))

    def __handle_serana_lydia_pattern(self, output: str, current_settings: sentence_generation_settings) -> tuple[SentenceContent | None, str]:
        """Handle special case for *Serana and *Lydia patterns - treat as speech"""
        # Find the end of the name (either space, punctuation, or end of string)
        name_end_match = re.search(r'(Serana|Lydia)(?=\s|$|[.!?;:,"])', output)
        if name_end_match:
            name_end_pos = name_end_match.end()
            name = name_end_match.group(1)

            # Extract the name part and replace * with empty and add :
            processed_name = name + ":"

            # Get the rest of the text after the name
            rest = output[name_end_pos:].lstrip()

            # Return the processed content as speech
            return SentenceContent(current_settings.current_speaker, processed_name, SentenceTypeEnum.SPEECH, False), rest

        # If no proper pattern found, fall back to normal processing
        return None, output
