...
class OtherDefinitions:
    ...
    def get_max_count_events_config_value() -> ConfigValue:
        max_count_events_description = """Maximum number of in-game events that are sent to the LLM per message (for both player and radiant conversations). 
                                    If the maximum number is reached, the oldest events will be dropped (most recent events are prioritized).
                                    For radiant conversations, events are queued between conversations to ensure none are lost.
