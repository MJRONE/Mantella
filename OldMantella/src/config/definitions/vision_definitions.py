...
class VisionDefinitions:
    ...
    def get_vision_timeout_config_value() -> ConfigValue:
        description = """Maximum time in seconds to wait for the vision model to respond when generating descriptions for radiant conversations.
                        Increase this value if the vision model is slow or unreliable. Decrease it if you want faster fallback to regular conversation flow."""
        return ConfigValueInt("vision_timeout", "Vision Timeout (seconds)", description, 12, 5, 60, tags=[ConfigValueTag.advanced])

    @staticmethod
    def get_periodic_vision_interval_config_value() -> ConfigValue:
        description = """Time interval in seconds between periodic vision updates during radiant conversations.
                        Set to 0 to disable periodic vision updates. Higher values reduce API calls but may make conversations less responsive to visual changes."""
        return ConfigValueInt("periodic_vision_interval", "Periodic Vision Interval (seconds)", description, 25, 0, 300, tags=[ConfigValueTag.advanced])

    @staticmethod
    def get_image_transcription_prefix_config_value() -> ConfigValue:
        description = """Custom text that will be prepended to the image transcription before it is passed to the LLM.
                        This can be used to provide additional context or hints about the image content."""
        default_value = "The image is the view of MJR. Blonde women in bikini are likely Serana or Lydia"
        return ConfigValueString("image_transcription_prefix", "Image Transcription Prefix", description, default_value, tags=[ConfigValueTag.advanced])