...
class Context:
    ...
    def __init__(self, world_id: str, config: ConfigLoader, client: LLMClient, rememberer: Remembering, language: dict[Hashable, str]) -> None:
        ...
        self.__radiant_event_log: list[str] = []  # Persistent log for radiant conversations
    ...
    def get_radiant_event_log(self) -> list[str]:
        """Get all events accumulated for radiant conversations"""
        return self.__radiant_event_log.copy()
    
    @utils.time_it
    def clear_radiant_event_log(self):
        """Clear the radiant event log after it's been used (OPTIMIZED: reduced logging)"""
        event_count = len(self.__radiant_event_log)
        if event_count > 0:
            logging.debug(f"[RADIANT EVENTS] Clearing {event_count} used events from log")
        self.__radiant_event_log.clear()
    
    @utils.time_it
    def add_to_radiant_event_log(self, events: list[str]):
        """Add events to the persistent radiant log (with deduplication and max_count_events limit) - OPTIMIZED"""
        if not events:
            return
        
        old_count = len(self.__radiant_event_log)
        
        # Deduplicate: only add events that are not already in the log
        existing_events_set = set(self.__radiant_event_log)
        new_unique_events = [event for event in events if event not in existing_events_set]
        
        if not new_unique_events:
            logging.debug(f"[RADIANT EVENTS] All {len(events)} events were duplicates, skipped")
            return
        
        # Add new unique events
        self.__radiant_event_log.extend(new_unique_events)
        
        # Apply max_count_events limit, keeping the most recent events
        ...
        if max_events > 0 and len(self.__radiant_event_log) > max_events:
            ...
            logging.debug(f"[RADIANT EVENTS] Trimmed to {max_events} most recent events")
        
        # Single optimized log statement
        logging.info(f"[RADIANT EVENTS] Added {len(new_unique_events)} events (total: {len(self.__radiant_event_log)})")
    ...
    def update_context(self, location: str | None, in_game_time: int | None, custom_ingame_events: list[str] | None, weather: str, custom_context_values: dict[str, Any]):
        ...
        if custom_ingame_events:
            ...
            # Also add to radiant event log for persistent storage (OPTIMIZED: logging moved to add_to_radiant_event_log)
    ...
    def __get_trusts(self) -> str:
        ...
        """
        # For radiant conversations (no player), skip trust calculation
        if not self.npcs_in_conversation.contains_player_character():
            return ""
