...
class Summaries(Remembering):
    ...
    @utils.time_it
    def save_conversation_state(self, messages: message_thread, npcs_in_conversation: Characters, world_id: str, is_reload=False, event_log: list[str] | None = None):
        summary = ''
        # Check if this is a radiant conversation (no player character)
        is_radiant = not npcs_in_conversation.contains_player_character()
        ...
        for npc in npcs_in_conversation.get_all_characters():
            if not npc.is_player_character:
                if len(summary) < 1: # if a summary has not already been generated, make one
                    summary = self.__create_new_conversation_summary(messages, npc.name, event_log if is_radiant else None)
    ...
    @utils.time_it
    def __create_new_conversation_summary(self, messages: message_thread, npc_name: str, event_log: list[str] | None = None) -> str:
        prompt = self.__memory_prompt.format(
                    name=npc_name,
                    language=self.__language_name,
                    game=self.__game
                )
        max_retries = 3  # Limit retries to prevent indefinite hanging
        retry_count = 0
        while retry_count < max_retries:
            try:
                if len(messages) >= 5:
                    text_to_summarize = messages.transform_to_dict_representation(messages.get_talk_only())
                    
                    # For radiant conversations, append event log to the text being summarized
                    if event_log and len(event_log) > 0:
                        events_text = "\n\nIn-game events that occurred during this conversation:\n" + "\n".join(f"- {event}" for event in event_log)
                        text_to_summarize += events_text
                        logging.info(f"[RADIANT SUMMARY] Adding {len(event_log)} events to conversation summary for {npc_name}")
                    
                    return self.summarize_conversation(text_to_summarize, prompt, npc_name)
                else:
                ...
                if retry_count >= max_retries:
                    logging.error(f'Failed to summarize conversation after {max_retries} attempts: {e}')
                    break
                logging.error(f'Failed to summarize conversation (attempt {retry_count}/{max_retries}). Retrying...')
        ...
        if count_tokens_summaries > summary_limit:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    retry_count += 1
                    if retry_count >= max_retries:
                        logging.error(f'Failed to resummarize conversation after {max_retries} attempts: {e}')
                        # Continue without resummarization if it fails
                        long_conversation_summary = conversation_summaries
                        break
                    logging.error(f'Failed to resummarize conversation (attempt {retry_count}/{max_retries}). Retrying...')
                    time.sleep(2)  # Reduced from 5s to 2s
                    continue
